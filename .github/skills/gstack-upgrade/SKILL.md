...
[ -z "$_AUTO" ] && _AUTO=$(.github/skills/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
.github/skills/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
.github/skills/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `.github/skills/bin/gstack-config set update_check true` to re-enable."
...
_TEAM_MODE=$(.github/skills/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
.github/skills/bin/gstack-update-check --force 2>/dev/null || \
