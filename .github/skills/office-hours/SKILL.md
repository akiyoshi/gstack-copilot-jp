...
```bash
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' | "$(git rev-parse --show-toplevel)/bin/gstack-developer-profile" --append-session
```
...
{If second opinion ran in Phase 3.5 (Outside Voice or Primary subagent): independent cold read — steelman, key insight, challenged premise, prototype suggestion. Verbatim or close paraphrase. If second opinion did NOT run (skipped or unavailable): omit this section entirely — do not include it.}
...
## Cross-Model Perspective
{If second opinion ran in Phase 3.5 (Outside Voice or Primary subagent): independent cold read — coolest version, key insight, existing tools, prototype suggestion. Verbatim or close paraphrase. If second opinion did NOT run (skipped or unavailable): omit this section entirely — do not include it.}
...
```bash
PROFILE=$("$(git rev-parse --show-toplevel)/bin/gstack-developer-profile" --read 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
```bash
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' | "$(git rev-parse --show-toplevel)/bin/gstack-developer-profile" --append-session
```
