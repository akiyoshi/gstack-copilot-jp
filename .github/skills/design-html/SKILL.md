...
_CEO=$(ls -t ./ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t ./designs/*/approved.json 2>/dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t ./designs/*/variant-*.png 2>/dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t ./designs/*/finalized.html 2>/dev/null | head -1)
...
- If variant PNGs found: show them inline using the view tool.
...
1. If `design binary (not available)` is available (`DESIGN_READY`), extract a structured implementation spec:
```bash
```
This returns colors, typography, layout structure, and component inventory via GPT-4o vision.

2. If `design binary (not available)` is not available, read the approved PNG inline using the view tool.
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -z "$_PRETEXT_VENDOR" ] && [ -f .github/skills/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=.github/skills/design-html/vendor/pretext.js
...
Write a single file using the create tool. Save to:
`./designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`./designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`
...
Show all three screenshots inline using the view tool. Check for:
...
LOOP:
  ...
  2. If an approved mockup PNG exists, show it inline (view tool) for visual comparison.
...
- **Surgical edits in the refinement loop.** Use the edit tool to make targeted changes,
  not the create tool to regenerate the entire file. The user may have made manual edits
