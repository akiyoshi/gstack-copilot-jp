...
checks, and deploy status commands — then persist everything to copilot-instructions.md.

After this runs once, `/land-and-deploy` reads copilot-instructions.md and skips detection entirely.
...
```bash
grep -A 20 "## Deploy Configuration" copilot-instructions.md 2>/dev/null || echo "NO_CONFIG"
```

If configuration already exists, show it and ask:

- **Context:** Deploy configuration already exists in copilot-instructions.md.
...
Read copilot-instructions.md (or create it). Find and replace the `## Deploy Configuration` section
...
Saved to copilot-instructions.md. /land-and-deploy will use these settings automatically.

Next steps:
- Run /land-and-deploy to merge and deploy your current PR
- Edit the "## Deploy Configuration" section in copilot-instructions.md to change settings
...
- **copilot-instructions.md is the source of truth.** All configuration lives there — not in a separate config file.
