...
DESIGN.md または plan.md が存在する場合、差分がプランを満たしているか検証する:

1. DESIGN.md の「提案するアプローチ」セクションから実装項目を抽出
2. 差分のファイル・関数と照合し、実装漏れがないか確認
3. 実装漏れを検出した場合 → ユーザーに報告:
   「プラン完了監査: DESIGN.md に記載の [項目] が差分に見つからない。意図的なスコープ外か？」
4. プラン文書がない場合 → スキップ
