...
For Design Review: run `source <(.github/skills/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**Dispatch this step as a subagent** using the task tool with `subagent_type: "general-purpose"`. The subagent runs the coverage audit in a fresh context window — the parent only sees the conclusion, not intermediate file reads. This is context-rot defense.
...
**Dispatch this step as a subagent** using the task tool with `subagent_type: "general-purpose"`. The subagent reads the plan file and every referenced code file in its own fresh context. Parent gets only the conclusion.
...
for PLAN_DIR in "$HOME/.gstack/projects/$_PLAN_SLUG" "$HOME/.gstack/plans"  ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
3. If `deferred > 0` and no user override, present the deferred items via ask_user before continuing.
...
```bash
source <(.github/skills/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
```bash
.github/skills/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
TMPERR_DRL=$(mktemp /tmp/outside-voice-drl-XXXXXXXX)
...
Present Outside Voice output under a `OUTSIDE VOICE (design):` header, merged with the checklist findings above.
...
**Dispatch the fetch + classification as a subagent** using the task tool with `subagent_type: "general-purpose"`. The subagent pulls every Greptile comment, runs the escalation detection algorithm, and classifies each comment. Parent receives a structured list and handles user interaction + file edits.
...
Every diff gets adversarial review from both Claude and Outside Voice. LOC is not a proxy for risk — a 5-line auth change can be critical.
...
- **Empty response:** "Outside Voice unavailable. Stderr: <paste relevant error>."
...
Set the bash tool's `timeout` parameter to `300000` (5 minutes). Do NOT use the `timeout` shell command — it doesn't exist on macOS. Present output under `OUTSIDE VOICE SAYS (code review):` header.
...
```
Substitute: STATUS = "clean" if no findings across ALL passes, "issues_found" if any pass found issues. SOURCE = "both" if Outside Voice ran, "claude" if only Primary subagent ran. GATE = the Outside Voice structured review gate result ("pass"/"fail"), "skipped" if diff < 200, or "informational" if Outside Voice was unavailable. If all passes failed, do NOT persist.
...
════════════════════════════════════════════════════════════
  ...
  Models used: Claude structured ✓  Claude adversarial ✓/✗  Outside Voice ✓/✗
════════════════════════════════════════════════════════════
```
...
3. **Queue-aware version pick (workspace-aware ship, v1.6.4.0+).** Call `bin/gstack-next-version` to see what's already claimed by open PRs + active sibling Conductor worktrees, then render the queue state to the user:
   ...
   - If `CLAIMED_COUNT > 0`: render the queue table to the user so they can see landing order at a glance:
     ...
     ```
   - If `ACTIVE_SIBLING_COUNT > 0` and any active sibling's VERSION is `>= NEW_VERSION`, use **ask_user**: "Sibling workspace <path> has v<X> committed <N>h ago but hasn't PR'd yet. Wait for them to ship first, or advance past? A) Advance past (recommended for unrelated work), B) Abort /ship and sync up with sibling first."
...
**Anti-footgun rules:**
- NEVER blind `git reset --soft` if there are non-WIP commits. Outside Voice flagged this
