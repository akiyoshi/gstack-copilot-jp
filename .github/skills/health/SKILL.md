...
Read copilot-instructions.md and look for a `## Health Stack` section. If found, parse the tools
...
A) Looks right -- persist to copilot-instructions.md and continue
...
Read the last 10 entries from `./health-history.jsonl` (if the
...
3. **Respect copilot-instructions.md.** If `## Health Stack` is configured, use those exact commands. Do not second-guess.
