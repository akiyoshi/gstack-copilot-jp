...
_PREV=$(find ./designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
echo "$_PREV"
```
...
**Persistent taste profile (v1 schema at `./taste-profile.json`):**
...
variant, call `.github/skills/bin/gstack-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `.github/skills/bin/gstack-taste-update rejected <variant-path>`.
...
**Important: [design binary: not available in Copilot CLI] propagation.** The `design binary (not available)` variable from DESIGN SETUP is a shell
...
Design binary: {absolute path to [design binary: not available in Copilot CLI]}
Brief: {the full variant-specific brief for this direction}
Output: /tmp/variant-{letter}.png
Final location: {_DESIGN_DIR absolute path}/variant-{letter}.png

Steps:
1. Run: design-binary generate --brief "{brief}" --output /tmp/variant-{letter}.png
2. If the command fails with a rate limit error (429 or "rate limit"), wait 5 seconds
   and retry. Up to 3 retries.
3. If the output file is missing or empty after the command succeeds, retry once.
4. Copy: cp /tmp/variant-{letter}.png {_DESIGN_DIR}/variant-{letter}.png
5. Quality check: design-binary check --image {_DESIGN_DIR}/variant-{letter}.png --brief "{brief}"
...
```
design-binary evolve --screenshot {_DESIGN_DIR}/current.png --brief "{brief}" --output /tmp/variant-{letter}.png
```
...
1. Read each generated PNG inline (view tool) so the user sees all variants at once.
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `./designs/`. This is enforced. See DESIGN_SETUP above.
